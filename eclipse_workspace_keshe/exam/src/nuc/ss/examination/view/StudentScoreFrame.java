...
public class StudentScoreFrame extends BaseFrame {
	...
	protected void setup() {
		...
		Score score = studentScoreService.getScore(user);
		for (int row = 0; row < 1; row++) {
			Vector<String> rowV = new Vector<>();
			rowV.add(score.getAccount());
			rowV.add(score.getName());
			rowV.add(score.getData());
			rowV.add(score.getScore()+"");
			if(score.getQuestionScore() != 0.0) {
				rowV.add(score.getQuestionScore()+"");
			}else {
				rowV.add("未判");
			}
